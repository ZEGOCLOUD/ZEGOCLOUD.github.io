...
const config = {
  ...
  serverURL: "wss://webliveroom1719562607-api.zegocloud.com/ws",
...
function generateToken() {
  ...
  return fetch(
    `https://easy-example-call.herokuapp.com/access_token?uid=${config.userID}`,
    {
      method: "GET",
    }
  ).then((data) => data.json());
...
function initSDK() {
  ...
  ZegoExpressManager.shared.onInvitedJoinLive(() => {
    showNotify(
      "InvitedJoinLive",
      "Received the invitation of taking the co-host seat, are you sure you want to connect?",
      (result) => {
...
async function joinLiveAsHost() {
  ...
  ZegoExpressManager.shared
    .joinRoom(roomID, token, user, [1, 2, 4, 8])
    .then((result) => {
      if (result) {
    ...
    .catch((error) => {
      if (error.code === 1002034) {
        alert(
          "The number of users logged in to the room exceeds the maximum number of users configured in the room"
        );
      }
    });
}
async function joinLiveAsAudience() {
  ...
  ZegoExpressManager.shared
    .joinRoom(roomID, token, user, [1, 2])
    .then((result) => {
      if (result) {
        triggerPageView("main");
...
function enableCamera(userID) {
  ...
  if (
    (ownRole === 0 && userID === undefined) ||
    (ownRole !== 0 && userID === config.userID)
  ) {
...
function sendBroadcastMessage() {
  ...
  ZegoExpressManager.shared
    .sendBroadcastMessage(inputView.value)
    .then((result) => {
      if (result) {
        console.warn("[ZEGOCLOUD Log][Demo][sendBroadcastMessage] - Success");
        updateMsgListView([
          {
            fromUser: { userID: config.userID, userName: config.userName },
            message: inputView.value,
          },
        ]);
        inputView.value = "";
      }
...
document
  .querySelector("#message-input-con input")
...
function triggerRenderHostViewCon(show, userID, videoDom) {
  ...
  if (show) {
    ...
    const hostConView = hostListView.querySelector(
      "[id^=host-con" + (userID || "") + "]"
    );
...
function triggerRenderCoHostViewCon(show, userID, videoDom) {
  ...
  if (show) {
    ...
    if (
      data.userList.get(config.userID).role === 0 ||
      ...
      operateConView.innerHTML =
        ...
        "<img onclick=\"enableCamera('" +
        userID +
        '\')" src="./img/icon_camera.png" alt="camera" />' +
        "</div>" +
        '<div class="mic-con small">' +
        "<img onclick=\"enableMic('" +
        userID +
        '\')" src="./img/icon_mic.png" alt="mic" />' +
        "</div>";
    ...
    const cohostConViewList = coHostListView.querySelectorAll(
      "[id^=cohost-con" + (userID || "") + "]"
    );
    ...
    if (updateType === "ADD") {
      ...
      if (ownRole === 0 && userID !== config.userID) {
        userItemView.innerHTML =
          ...
          "(" +
          roleName +
          ')</span><span class="item-btn" onclick="inviteJoinLive(\'' +
          userID +
          "')\">invite</span></div>";
      } else {
        userItemView.innerHTML =
      ...
      role === 0
        ? userListView.append(userItemView)
        : userListView.appendChild(userItemView);
    } else if (updateType === "UPDATE") {
      // Update
      const itemTextView = userListView.querySelector(
        ".user-item" + userID + " .item-text"
      );
      itemTextView.innerHTML = itemTextView.innerHTML.replace(
        /\((\S+)\)/gi,
        "(" + roleName + ")"
      );
    } else {
...
function updateMsgListView(msgList) {
  ...
  msgList.forEach((msg) => {
    ...
    messageItemView.innerHTML =
      '<div class="message-item"><span>' +
